...
//Luego se busca el m铆nimo recorriendo los relacionados de k-i para todo i en relacionados
...
vector<int> tsp_cota(const Grafo<peso_t>& g, int (*cota)(vector<int>&, const Grafo<int>&)) {
  ...
  while((!cola.empty()) && (cola.top().second < mejor_longitud)){
    ...
    cola.pop();
    // No podemos formar directamente una soluci贸n
    ...
    else{// Podemos formar una soluci贸n
      for(int i = 0; i < g.numNodos(); i++){
        if(find(camino_actual.begin(), camino_actual.end(), i) == camino_actual.end()){
...
vector<int> tsp_backtracking(const Grafo<peso_t>& g) {
	...
	vector<int> solucion; // se modifica 茅ste
...
int ejecutar(vector<int> (*f)(const Grafo<peso_t>&), const Grafo<peso_t>& g, ostream& fo) {
  ...
  */
  ...
  cout << "N鷐ero nodos expandidos: " << num_nodos_expandidos_3 << endl;
  cout << "N鷐ero de veces que se poda: " << num_veces_poda_3 << endl;
  cout << "Tama駉 m醲imo de la cola con prioridad de nodos vivos: " << "----" << endl;
...
int main(int argc, char * argv[])
{
  ...
  cout << "Longitud 贸ptima: " << longitud_desde_archivo(nombre_optimo(argv[1]), g) << endl;
  return 0;
}
